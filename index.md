...
##第三方引用
- Spring 3.2.13：主要应用IOC、AOP、事件、JDBC、事务管理、定时调度
- druid 1.2.4：数据库连接池
- fastjson 1.1.46：JSON格式化工具
- log4j 2.3：日志工具
- guava 18.0：内部缓存
- ExtJS 4.2：前端展示
##运行环境
...
> 
> 绿色版下载地址：http://mdd.lingx.com/20210115/lingx-mdd.zip
...
4. 树型数据结构表，上级字段名必须用fid，有支持icon_cls（图标样式）,state（展开open、关闭close）字段

# 模型架构
> 主要有模型、属性、方法、验证器、解析器、执行器
## 模型
...
### 表模型
表模型即为数据库表的模型模型，表字段为模型属性，字段名称为属性代码，字段注释为属性名称；默认情况下字段与属性一一映射，特殊情况下可以多个属性映射同一字段，或没有属性来映射这段。
...
### 视图模型
视图模型即为数据库视图的模型模型，只是引用模型不同。在平台的使用方式并无二致。
视图名称建议以“v” 为前缀。
...
**指向模型模型**必须为“tlingx_optionitem” 字典子项
### 树型属性
...
- **是否转义** 在指向模型模型的字段，在显示时默认会转义。比如：用户性别在数据库中值为1，在显示的时候会跟会字典的配置转化成男。
...
> 在要进行下面操作时，需要在菜单“平台管理”-“模型管理”，在列表找到“用户管理”、“组织管理”；将其状态变更为“开发”。再退出重新登录
## 列表模版
...
2、动态：在表单设计器中，设置【指向模型模型】在表单中可以直接动态调取该模型的combo方法，注意授权

同样也可以引用字典，设置【指向模型模型】为【选项子项】，在扩展参数中填入字典代码。
...
###列表控件
列表数据控件，原本“外部链接”就可以支持列表控件（通过iframe内嵌模型的gird方法），目前列表的内部控件只支持文件框；开发本控件是为了结合手机端流程的列表数据统一管理。
...
模型、属性、方法、验证器、解释器、执行器等，都有自己的属性。这里是相应的属性设置区
###代码编辑区
